...
docker exec facebook-news-n8n sh -c 'grep -o trace-v8-webhook-direct /tmp/workflow.json | head -1' || echo "(import file check failed)"
docker exec facebook-news-n8n sh -c 'grep -c webhookDirectToSlot /tmp/workflow.json' || true
