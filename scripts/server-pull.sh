...
HAS_V7=$(grep -c 'trace-v10-feed-publish' workflow/facebook-us-news-automation.json || echo 0)
HAS_GATE=$(grep -c '"name": "publishFeedLink"' workflow/facebook-us-news-automation.json || echo 0)
...
if [ "${HAS_V7:-0}" -lt 1 ]; then
  echo ""
  echo "[!!] ERROR: Workflow on disk is OLD (need trace-v10-feed-publish)."
