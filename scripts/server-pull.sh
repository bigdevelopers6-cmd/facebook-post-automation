...
HAS_V7=$(grep -c 'trace-v9-webhook-fastpublish' workflow/facebook-us-news-automation.json || echo 0)
HAS_GATE=$(grep -c '"name": "webhookSlotFast"' workflow/facebook-us-news-automation.json || echo 0)
...
echo "webhookSlotFast node (want >=1): $HAS_GATE"
echo "webhookSetup typeVersion 1 (want >=1): $CODE_V1"
echo "meta: $BUILD"

if [ "${HAS_V7:-0}" -lt 1 ]; then
  echo ""
  echo "[!!] ERROR: Workflow on disk is OLD (need trace-v9-webhook-fastpublish)."
