...
WORKFLOW_BUILD_EXPECT="${WORKFLOW_BUILD_EXPECT:-2026-05-30-trace-v7d-split-bypass}"
...
preflight() {
  ...
  v7=$(grep -c 'trace-v7d-split-bypass' workflow/facebook-us-news-automation.json 2>/dev/null || echo 0)
  echo "Workflow: trace-v7d=$v7 pipelineLog-fn=$markers (want v7d>=1, fn=0)"
  if [ "${v7:-0}" -lt 1 ]; then
